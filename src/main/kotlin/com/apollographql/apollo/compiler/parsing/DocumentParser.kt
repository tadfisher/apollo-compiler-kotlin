...
import com.apollographql.apollo.compiler.parsing.GraphqlParser.*

fun DocumentContext.toAst(): Document {
    return Document(definitions = definition().map { it.toAst() }, location = astLocation)
}
...
fun OperationDefinitionContext.toAst() = OperationDefinition(
        operationType = operationType()?.toAst() ?: OperationType.Query,
        name = Name().text,
...
fun ValueContext.toAst(): Value {
    return variable()?.toAst()
            ?: IntValue()?.let { IntValue(value = it.symbol.text.toInt(), location = astLocation) }
            ?: FloatValue()?.let { FloatValue(value = it.symbol.text.toDouble(), location = astLocation) }
            ?: StringValue()?.let { StringValue(value = it.symbol.text, location = astLocation) }
            ?: booleanValue()?.toAst()
            ?: nullValue()?.toAst()
            ?: enumValue()?.toAst()
            ?: listValue()?.toAst()
...
fun Value_constContext.toAst(): Value {
    return IntValue()?.let { IntValue(value = it.symbol.text.toInt(), location = astLocation) }
            ?: FloatValue()?.let { FloatValue(value = it.symbol.text.toDouble(), location = astLocation) }
            ?: StringValue()?.let { StringValue(value = it.symbol.text, location = astLocation) }
            ?: booleanValue()?.toAst()
            ?: nullValue()?.toAst()
...
fun DescriptionContext.toAst(): StringValue {
    return if (text.startsWith("\"\"\"") && text.endsWith("\"\"\"")) {
        ...
        StringValue(value = text.removeSurrounding("\""), location = astLocation)
    }
}
